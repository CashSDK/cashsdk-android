...
/**
 * The server recorded the purchase but did not confirm the access it should grant
 * (`purchaseOutcomeConfirmed: false`). Still GRANTED as far as caching and settling go: the
 * snapshot is this user's, and Google refunds a purchase nobody acknowledges. `purchase()` and
 * restore return normally with the flag set, so hosts that check it (and the SDK's own paywall)
 * can run their recovery instead of reporting a success that unlocked nothing. `null` (an older
 * server, or evidence the server could not read) counts as confirmed, as it always has.
 */
internal val Entitlements.accessUnconfirmed: Boolean get() = purchaseOutcomeConfirmed == false
