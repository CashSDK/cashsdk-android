...
sealed class CashSDKError(
     ...
     * completed sale.
     *
     * `purchase()` also throws this, before any charge, when the Google account already owns
     * the product (Play's ITEM_ALREADY_OWNED, or a subscription the account already holds) and
     * the server keeps that purchase with another app account. Treat it as "already owned:
     * restore, or sign in to the account that owns it", not as a new purchase to retry.
     */
     ...
     *
     * For non-2xx answers [code] is read from either error shape the API sends,
     * `{"error":"invalid_purchase"}` or `{"error":{"code":"rate_limited"}}`.
     */
