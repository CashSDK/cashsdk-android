    ...
    MaterialTheme(colorScheme = if (dark) darkColorScheme() else lightColorScheme()) {
        ...
        fun buy(productId: String?) {
            ...
            scope.launch {
                ...
                errorText = null
                try {
                    val result = client.paywallPurchase(activity, price)
                    if (result.accessUnconfirmed) {
                        // Paid and settled, but the server did not confirm the access: not a
                        // success, and not something to buy again. Stay here and say so.
                        errorText = UNCONFIRMED_PURCHASE_TEXT
                        client.track(
                    ...
                    // revenue to an experiment arm server-side. A purchase the account already
                    // owned was not bought here, so it is not counted as one.
                    client.track(
                        if (result.alreadyOwned) "purchase_already_owned" else "purchase_success",
                        placement = presentation.placement,
                        product = productId,
                        variant = presentation.variantId,
                    )
                    ...
                    errorText = PENDING_PURCHASE_TEXT
