...
/**
 * The introductory phases of the offer this price will buy, or null. Trial wording ("Free for
 * 1 week") therefore only ever describes an offer that Play returned for this user (Play lists
 * offers only to eligible users) and that the paywall passes to `purchase`. A base plan has a
 * single phase, so the paywall's default price, which buys the base plan, never gets trial copy.
 */
