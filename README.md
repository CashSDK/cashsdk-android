...
### Option A — Maven artifact ⚠️ NOT PUBLISHED YET

```kotlin
// app/build.gradle.kts — this coordinate does NOT resolve today
dependencies {
    implementation("com.cashsdk:cashsdk-android:1.0.0")
}
```

**This is the intended install path, but the artifact has not been published to Maven
Central.** Unlike Swift Package Manager, Gradle resolves Maven coordinates rather than git
tags, so the public
[github.com/cashsdk/cashsdk-android](https://github.com/cashsdk/cashsdk-android) repo does
not make this line work on its own. Publishing needs `com.cashsdk` namespace verification
plus `maven-publish` + signing config — see
[AGENTS/SDK-DISTRIBUTION.md](https://github.com/cashsdk/cashsdk-android) §3 in the monorepo.

Until then use **Option B** (source module), which works today. The `.aar` ships
`consumer-rules.pro`, so no extra R8 config is needed in the host app either way.

### Option B — source module (works today)
