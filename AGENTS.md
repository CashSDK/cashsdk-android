...
**The Maven coordinate is NOT published.** This does not resolve from anywhere:

```kotlin
implementation("com.cashsdk:cashsdk-android:1.0.0")   // ← does NOT exist yet
```

Unlike Swift Package Manager, Gradle resolves Maven artifacts rather than git tags, so the
public GitHub repo does not make that line work. **Do not emit it and tell the user it will
work.** The working install today is the source module:
